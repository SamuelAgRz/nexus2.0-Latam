...
- **Same-payload dimension binding (hard invariant, overrides everything below).** The `'Table'[Column]` key you emit for a matched value MUST be a hierarchy column drawn from the **same** `dimension_value_reference` payload in which that value was physically found — i.e. a column from that payload's own `hierarchy` array (normalized to quoted notation). A value found in the Customer payload can ONLY be keyed to a `'Ship To'` column; a value found in the Bottler payload ONLY to a `'Ship From'` column; a value found in the Product payload ONLY to a `'Product'` column; and so on. NEVER emit a value under a column belonging to a different dimension/table than the payload it was matched in — even if the same or a similar string appears to fit another dimension conceptually. This makes it structurally impossible to, e.g., report a customer/retailer name as a Trademark or Brand. If a value genuinely appears in more than one payload, see the cross-table match-quality ladder and `ambiguous_terms` rules below; if it appears in exactly one payload, that payload's dimension is the only permitted key.
- **Entity-type anchoring (highest priority among selection rules).** When the user's question names the *dimension type* of a value (bottler, bottler zone, brand / brand group, trademark, category, sub-category, channel, customer, package, container, zone, …), you MUST resolve that value **only** into the column belonging to that named type. Do NOT surface the value under a column from any other hierarchy/table, even if the value string also appears (or fuzzily matches) elsewhere. Use this noun → column map:
  ...
  - channel → `'Channel'[LT1.3 - Channel Macro Group]` (the coarsest level, used as the default for a generic channel reference; resolve to a finer `'Channel'` level — Channel Group, Trade Channel, Sub Trade Channel — only when the user explicitly names that level or names a specific channel value that belongs to it)
...
- **Include ALL in-scope values that match**, grouped under a **single chosen column** per term — except when the cross-table match-quality ladder below ties, in which case ALL tied columns carry the term's values.
  ...
  - **Cross-table match-quality ladder (no dimension type named).** If the user did NOT name a dimension type for a term, rank every match of that term across ALL tables by match quality: (1) exact whole-value match, (2) whole-word match, (3) substring match, (4) tolerant match. The "tables" considered are exactly the payloads in which the term actually appears — per the **Same-payload dimension binding** invariant, a term is only ever a candidate for the dimension(s) of the payload(s) it was literally found in; you may never introduce a table/column the value does not appear in. If the term appears in exactly one payload, resolve it into that payload's dimension (then apply the within-table level rules above) — do NOT reassign it to any other dimension. If two or more distinct payloads tie at the best quality, surface the term's matched values under ALL tied columns in `candidate_dimension_values` AND record the term with its tied columns in `ambiguous_terms` — surfaced alternatives are context for downstream selection, never obligatory conjunctive filters. Never silently drop a term because it matches in more than one table, and never fabricate a column for a term that matched in only one.
- Copy matched values **verbatim** — never translate, abbreviate, reorder, normalize, or invent values.
- **Matching decides WHICH value; it never rewrites the value (hard invariant).** Fuzzy / tolerant / case- / accent- / punctuation-insensitive matching (all tiers above) is used ONLY to decide which stored payload value a user term maps to. Once matched, the value written into `candidate_dimension_values` is ALWAYS the stored payload string copied **character-for-character** — every hyphen, period, ampersand, internal space, and the stored casing preserved exactly. NEVER emit the user's spelling and NEVER emit a punctuation-normalized form of the stored value. Example: the user term "Walmart" matches (tolerantly) the stored `Wal-Mart Inc.`; the emitted value MUST be `Wal-Mart Inc.`, NEVER `Walmart Inc.`. A loose match never licenses a loose value.
...
Worked example — a retailer/customer name that exists ONLY in the Customer payload. Suppose the user names one or more retailer entities with no dimension type ("... across <RetailerA>, <RetailerB>, <RetailerC>") and each of those values is found only in the `customer_reference` payload (the Customer dimension), while a bottler entity in the same question is found only in the `shipfrom_reference` payload:

```
"candidate_dimension_values": {
  "'Ship To'[LT1.2 - Customer]": ["<RetailerA>", "<RetailerB>", "<RetailerC>"],
  "'Ship From'[L1.3 - Bottler]": ["<Bottler>"]
}
```

Each retailer resolves to `'Ship To'[LT1.2 - Customer]` because that is the only payload it appears in (Same-payload dimension binding). It MUST NEVER be keyed to `'Product'[LT1.3 - Trademark Category]` or any other Product/Channel/Package column, even though a retailer name can superficially read like a brand or trademark. The bottler stays on its own `'Ship From'` column. This resolution must be identical on every run — the binding is derived from where the value lives in the payloads, not from model judgment.
...
- Never invent, translate, or normalize values (only keys are notation-normalized — values are copied verbatim); never add a column that is not in a retrieved reference hierarchy. This applies to punctuation and hyphens specifically: a value stored as `Wal-Mart Inc.` is emitted as `Wal-Mart Inc.` (with the hyphen and trailing period), never as `Walmart Inc.`, regardless of how the user spelled the term that matched it. Downstream applies the value as-is, so a dropped hyphen makes the cube filter match nothing.
